...
## 📝 50. 入力した価格以上の商品を表示し、合計金額を計算する（`stretch50.rb`）
...
キーボードから基準となる価格を入力してください。

入力した価格以上の商品だけを、次の形式で表示します。

```text
商品名：価格円
```

さらに、条件に合った商品の合計金額を最後に表示してください。

たとえば、`500` と入力した場合は、次のように表示されます。
...
> [!TIP]
> まずは `each`、`if`、変数 `total` を使って作れます。
>
> `select` を使って条件に合う商品を新しい配列へ取り出してから処理する方法を調べても構いません。
...
puts "基準となる価格を入力してください"
minimum_price = gets.to_i
