...
エラー文は多くの場合、英語で表示されます。最初は難しく感じるかもしれませんが、すべてを完璧に理解する必要はありません。分かる単語を少しずつ拾うだけでも、解決のヒントが見えてきます。

たとえば、次のような単語がよく出てきます。

- `unexpected`（アンエクスペクテッド）: 想定外のものがある
- `missing`（ミッシング）: 何かが足りない
- `undefined`（アンディファインド）: 定義されていない

エラー文には、「どこで」「何が」問題なのかが書かれています。つまり、エラー文そのものが解決の手がかりです。
...
困ったときほど、まずはエラー文を落ち着いて読みましょう。最初から一発で正しく書ける人はいません。エラーを出して修正すること自体が、よい練習になります。
