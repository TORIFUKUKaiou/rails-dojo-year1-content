...
- `推奨（余裕がある人）`：4〜6 に進む（文字列の比較、組み合わせ、入力）
...
このチャレンジ問題では、できるだけ `gets` を使って、キーボードから入力する形で書いてみましょう。
...
値段と個数を入力して、合計金額を表示し、1000円以上なら「送料無料です」、それ未満なら「送料500円かかります」と表示するプログラムを作りましょう。
