...
上から順に見て、最初に正しい `score >= 60` で止まります。
...
問題43を直して、`weather` が `"晴れ"` のとき「散歩に行こう」と表示されるようにしましょう。
...
`age` を使って、6歳未満なら「幼児」、12歳未満なら「小学生くらい」、18歳未満なら「中高生くらい」、それ以外は「大人」と表示しましょう。
...
`age` を使って、6歳未満なら0円、18歳未満なら500円、それ以外は1000円と表示しましょう。
...
`hp` が0以下なら「たおれた」、30未満なら「危険」、70未満なら「注意」、それ以外は「元気」と表示しましょう。
...
`temperature` が35以上なら「猛暑」、30以上なら「暑い」、20以上なら「快適」、10以上なら「寒い」、それ以外は「とても寒い」と表示しましょう。
...
`money` が1000円以上なら「ランチが買えます」、500円以上なら「パンが買えます」、150円以上なら「飲み物が買えます」、それ以外は「何も買えません」と表示しましょう。
...
`month` が3〜5なら「春」、6〜8なら「夏」、9〜11なら「秋」、12・1・2なら「冬」と表示しましょう。
...
`day` が `"土曜"` または `"日曜"` なら「休み」、それ以外なら「平日」と表示しましょう。
...
`password` が `"ruby"` なら「ログイン成功」、それ以外なら「ログイン失敗」と表示しましょう。
