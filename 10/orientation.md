...
レスポンスとして返ってくるものの中で、Webページの画面を作る中心になるのがHTMLです。

HTMLは、ブラウザが読んで画面を表示するための文書です。
...
今回は、Webページの外側の形を読むために、代表的な要素を学びます。
...
| `<html>` | HTML全体を囲む |
| `<head>` | ページの情報を書く場所 |
| `<body>` | ブラウザの画面に表示される中身を書く場所 |
| `<h1>` | いちばん大きな見出し |
| `<h2>` | 2番目に大きな見出し |
| `<h3>` | 3番目に大きな見出し |
| `<p>` | 文章の段落 |

`<head>` の中身は、画面に表示される本文ではありません。

画面に見える見出しや文章は、主に `<body>` の中に書かれます。

ブラウザでWebページを開いたあと、右クリックして `ページのソースを表示` を選ぶと、そのページのHTMLを確認できます。

ソースを開いたら、次の文字を探してみてください。
...
> [!IMPORTANT]
> 今日は、HTMLの外側の形と、見出しや本文がどこに書かれているかを確認します。
> ソースを表示したら、`html`、`head`、`body`、`h1`、`p` を探してください。
...
今回は、ルーティングの細かい書き方ではなく、リクエストと処理のつながりを読みます。

まず、次の2行で、リクエストと処理のつながりを確認しましょう。
