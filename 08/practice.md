...
練習に進む前に、次の Visual 教材で、クラスとインスタンスの仕組みや、`initialize` メソッドとインスタンス変数の動きを確認しましょう。

[Rails Dojo Visuals](https://torifukukaiou.github.io/rails-dojo-visuals/)

画面上部の `第8回 クラス` を押してから進めてください。

使い方：

1. 右下の `>` ボタンを押すと、コードを1行ずつステップ実行できます。
2. 右側のコード、左側の図、下の説明文を見比べながら進めてください。
3. 左端にカーソルを合わせると、別のテーマを選ぶメニューが表示されます。

特に、`Article.new` から `initialize` が呼び出される動きや、インスタンス変数（`@` から始まる変数）がインスタンスの中に保存されてメソッドで使われる仕組みは、ここで一度見てから練習に進みましょう。
