...
練習に進む前に、次の Visual 教材で、メソッドの呼び出し・引数・戻り値・スコープの動きを確認しましょう。

[Rails Dojo Visuals](https://torifukukaiou.github.io/rails-dojo-visuals/)

画面上部の `第7回 メソッド` を押してから進めてください。

使い方：

1. 画面上部の `第7回 メソッド` を押します。
2. 右下の `>` ボタンを押すと、コードを1行ずつステップ実行できます。
3. 右側のコード、左側の図、下の説明文を見比べながら進めてください。

確認するポイント：

- `def` で作ったメソッドは、呼び出すまで実行されない。
- 引数を使うと、外側の値をメソッドの中へ渡せる。
- 戻り値は、メソッドの外側の変数に入れられる。
- `puts` で表示することと、戻り値として返すことは別の動きである。
- メソッドの中から外側の変数を直接見ることはできない。
- 外側の値を使いたいときは、引数として渡す。
